...
if (isset($LOG_CAMINHO)) {
    ...
    $identificacao=date("dmYHis")."-PID".getmypid()."-"."pessoas_processar";
    if(isset($LOG_NIVEL)) {
        if ($LOG_NIVEL>=1) {
...
$pessoas = array();
  ...
  $retorno = $progr->executarprogress("cadastros/app/1/pessoas_processar",json_encode($jsonEntrada));
  ...
  if (isset($pessoas["conteudoSaida"][0])) { // Conteudo Saida - Caso de erro
     ...
     if (!isset($pessoas["pessoas"][1])) {  // Verifica se tem mais de 1 registro
